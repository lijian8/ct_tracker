...
int main(int argc, char * argv[])
{
	...
	ct.init(last_gray, CTbox);

	vector<double> time_CT;
	time_CT.clear();
#endif
	...
	while (1)
	{
		...
		// Process Frame
		ct.processFrame(current_gray, CTbox);
		t1 = (double)cvGetTickCount() - t1;
		cout << "cost time: " << t1 / ((double)cvGetTickFrequency()*1000000.) <<" s"<< endl;
		cout<<"CT "<<cvRound(((double)cvGetTickFrequency()*1000000.)/t1)<<"FPS"<<endl;
#endif
...
#ifdef CT
		//putText(frame, "CT", Point(150, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 255), 3);
		rectangle(frame, CTbox, Scalar(255, 0, 255), 3);
#endif
